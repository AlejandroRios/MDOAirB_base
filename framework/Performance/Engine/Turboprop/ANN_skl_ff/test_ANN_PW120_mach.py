...
for i in altitude:
    for j in M0:
        ...
        FC_ANN = nn_unit.predict(scaler.transform([(i, j, 1)]))
...
# altitude_vec_model = np.load('Performance/Engine/Turboprop/ANN_skl_ff/altitude_model.npy')
...
ax1.plot(altitude_vec,FC_vec_model,'^b',alpha=0.5,label='PW120 model')
ax1.plot(altitude_vec,FC_vec_ANN, '-k',linewidth=2)

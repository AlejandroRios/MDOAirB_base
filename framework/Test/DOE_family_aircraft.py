...
def objective_function(x, original_vehicle, computation_mode, route_computation_mode, airports, distances, demands, index):
    ...
    try:
        ...
        if status == 0:
            ...
            # =============================================================================
            with open('Database/Family/40_to_100/airports/'+str(index)+'.pkl', 'wb') as f:
                pickle.dump(airports, f)

            with open('Database/Family/40_to_100/distances/'+str(index)+'.pkl', 'wb') as f:
                pickle.dump(distances, f)

            with open('Database/Family/40_to_100/demands/'+str(index)+'.pkl', 'wb') as f:
                pickle.dump(demands, f)

            with open('Database/Family/40_to_100/DOC/'+str(index)+'.pkl', 'wb') as f:
                pickle.dump(DOC_ik, f)

            with open('Database/Family/40_to_100/vehicle/'+str(index)+'.pkl', 'wb') as f:

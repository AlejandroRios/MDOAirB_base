...
def process_optimized_aircraft(design_variables, original_vehicle, computation_mode, route_computation_mode, airports, distances, demands, fixed_parameters):
    
    distances = {'FAD': {'FAD': 0, 'FAA': 800}, 'FAA': {'FAA': 0, 'FAD': 800}}
    
    def obj_func(x_local=np.array([155.,20.])):
        print("x0 = ", x_local[0])
        print("x1 = ", x_local[1])
        print("===========================================")
        X = [
            x_local[0],  #WingArea - x0
            10 * (42**2) / x_local[0],  #AspectRatio x 10 - x1
            30,  #TaperRatio - x2
            25,  #sweep_c4 - x3
            -2.25,  #twist - x4
    ...
    obj_opt, cons1_opt, cons2_opt = obj_func(xres)
    print("Best design variable = ", xres)
    print("Best Function value = ", obj_opt)
    print("Cons1 (TOFL) Function value = ", cons1_opt)
...
def main(argv):
    fixed_parameters = {}
    fixed_aircraft = {}
    customInputsfile = "Database/JsonSchema/05_FPO_Use_case.json" #Database/JsonSchema/Custom_Inputs_2D_single_traj.schema.json  #readArgv(argv)
    if not customInputsfile or not os.path.isfile(customInputsfile):
        print(f"Custom file {customInputsfile} does not exist")
        sys.exit(1)

    try:
        computation_mode, route_computation_mode, airports, distances, demands, design_variables, fixed_parameters, fixed_aircraft = read_custom_inputs(
    ...
    process_optimized_aircraft(design_variables, fixed_parameters, computation_mode, route_computation_mode, airports, distances, demands, fixed_parameters)

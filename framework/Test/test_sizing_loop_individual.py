...
def main(argv):
    ...
    df_output = pd.DataFrame([Y], columns=['MTOW', 'DOC', 'fuel_mass', 'total_mission_flight_time', 'mach', 'passenger_capacity',
                             'SAR', 'landing_field_length_computed', 'takeoff_field_length_computed', 'app_speed', 'status', 'design_status'])

    df_input.to_csv(
        "Results/analysis_single_input_1.csv", index=False)
    df_output.to_csv(
        "Results/analysis_single_output_1.csv", index=False)
